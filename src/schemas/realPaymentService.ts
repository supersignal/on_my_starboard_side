...
// 실제 취소 응답 타입
export interface RealCancelResponse {
  success: boolean;
  orderId: string;
  transactionId?: string;
  amount: number;
...
// 실제 결제 상태 조회 응답 타입
export interface RealPaymentStatusResponse {
  success: boolean;
  orderId: string;
  transactionId?: string;
  amount: number;
...
export class RealPaymentService {
  ...
  // 나이스페이먼츠 API 호출
  private async callNicePayAPI(endpoint: string, data: Record<string, any>): Promise<any> {
  ...
  // 실제 결제 요청 처리
  async processPayment(request: z.infer<typeof RealPaymentRequestZodSchema>): Promise<RealPaymentResponse> {
  ...
  // 실제 결제 취소 처리
  async processCancel(request: z.infer<typeof RealCancelRequestZodSchema>): Promise<RealCancelResponse> {
  ...
  // 실제 결제 상태 조회
  async getPaymentStatus(request: z.infer<typeof RealPaymentStatusZodSchema>): Promise<RealPaymentStatusResponse> {
  ...
  // 설정 업데이트
  updateConfig(newConfig: Partial<NicePayConfig>): void {
    this.config = { ...this.config, ...newConfig };
  }

  // 현재 설정 조회
  getConfig(): NicePayConfig {
    return { ...this.config };
  }
}

// 환경변수에서 설정을 로드하는 함수
export function createRealPaymentService(): RealPaymentService {
