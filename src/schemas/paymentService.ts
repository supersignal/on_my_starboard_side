...
export class PaymentTestService {
  private testPayments: Map<string, PaymentResponse> = new Map();
  private testCancels: Map<string, CancelResponse> = new Map();

  // 결제 요청 처리 (테스트용)
  async processPayment(request: z.infer<typeof PaymentRequestZodSchema>): Promise<PaymentResponse> {
  ...
  // 결제 취소 처리 (테스트용)
  async processCancel(request: z.infer<typeof CancelRequestZodSchema>): Promise<CancelResponse> {
  ...
  // 결제 상태 조회 (테스트용)
  async getPaymentStatus(request: z.infer<typeof PaymentStatusZodSchema>): Promise<PaymentStatusResponse> {
  ...
  // 테스트 데이터 조회
  getAllTestPayments(): PaymentResponse[] {
    return Array.from(this.testPayments.values());
  }

  getAllTestCancels(): CancelResponse[] {
