...
export class EnhancedBM25Calculator {
     ...
     * 문서 통계 정보 사전 계산
     */
    precomputeDocumentStats(documents) {
    ...
    calculateScores(query, documents, topN = 10) {
        ...
        for (const chunk of allChunks) {
            ...
            if (!stats)
                continue;
            const { score, matchedTerms, relevanceScore } = this.calculateChunkScore(queryTerms, chunk, stats);
            if (score > 0) {
                results.push({
                    id: chunk.id,
     ...
     * 개별 청크의 BM25 점수 계산
     */
    calculateChunkScore(queryTerms, chunk, stats) {
        let bm25Score = 0;
     ...
     * BM25 구성 요소 계산
     */
    calculateBM25Component(tf, docLength, avgDocLength, idf) {
     ...
     * 전역 용어 통계 업데이트
     */
    updateGlobalTermStats(termFreq, titleTermFreq) {
     ...
     * 동적 파라미터 조정
     */
    adjustParameters(queryLength, avgDocLength) {
        // 쿼리 길이에 따른 k1 조정
        if (queryLength > 5) {
     ...
     */
    findSimilarTerms(term, maxDistance = 1) {
        const similarTerms = [];
