...
export class HybridSearchEngine {
     ...
     */
    async precomputeEmbeddings(documents) {
    ...
    async search(query, documents, topN = 10) {
        ...
        // 2. BM25 검색
        const bm25Results = this.bm25Calculator.calculateScores(processedQuery.join(' '), documents, topN * 2 // 더 많은 후보를 가져와서 임베딩 점수와 결합
        );
        // 3. 임베딩 검색
        const queryEmbedding = await this.computeSimpleEmbedding(query, '');
        const embeddingResults = await this.computeEmbeddingScores(queryEmbedding, bm25Results.map(r => r.chunkId), topN * 2);
        // 4. 결과 결합
     ...
     */
    async computeSimpleEmbedding(text, title) {
     ...
     * 임베딩 유사도 점수 계산
     */
    async computeEmbeddingScores(queryEmbedding, chunkIds, topN) {
     ...
     * BM25와 임베딩 결과 결합
     */
    combineResults(bm25Results, embeddingScores) {
        const results = [];
        for (const bm25Result of bm25Results) {
            ...
            // 가중 평균으로 결합
            const combinedScore = normalizedBm25Score * this.config.bm25Weight +
     ...
     * 검색 성능 메트릭 계산
     */
    calculateSearchMetrics(results) {
        if (results.length === 0) {
