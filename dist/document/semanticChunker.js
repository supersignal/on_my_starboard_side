...
export class SemanticChunker {
    ...
    maxChunkSize;
    overlapSize;
    constructor(minChunkSize = 50, maxChunkSize = 500, overlapSize = 50) {
     ...
     * 마크다운 문서를 의미적으로 청킹
     */
    chunkDocument(markdown, documentId) {
     ...
     * 구조적 청킹 수행
     */
    extractStructuralChunks(ast, documentId) {
        const chunks = [];
        let chunkId = 0;
        let currentChunk = [];
        let currentTitle = '';
            ...
            switch (node.type) {
                case 'heading':
                    ...
                    if (currentChunk.length > 0) {
                        chunks.push(this.createChunk(`${documentId}_${chunkId++}`, currentChunk.join('\n'), currentTitle, currentType, currentLevel, documentId));
                        currentChunk = [];
                    }
                    // 새 청크 시작
                ...
                case 'code':
                    ...
                    chunks.push(this.createChunk(`${documentId}_${chunkId++}`, `\`\`\`${node.lang || ''}\n${node.value}\n\`\`\``, currentTitle, 'code', currentLevel, documentId));
                    break;
                case 'list':
                    // 리스트는 별도 청크로 처리
                    if (currentChunk.length > 0) {
                    ...
                    chunks.push(this.createChunk(`${documentId}_${chunkId++}`, listContent, currentTitle, 'list', currentLevel, documentId));
                    break;
                case 'table':
                    // 테이블은 별도 청크로 처리
                    if (currentChunk.length > 0) {
                    ...
                    chunks.push(this.createChunk(`${documentId}_${chunkId++}`, tableContent, currentTitle, 'table', currentLevel, documentId));
                    break;
                default:
        ...
        while (i < chunks.length) {
            ...
            if (currentChunk.type === 'heading') {
                ...
                if (relatedChunks.length > 1) {
                    ...
                    const combinedChunk = this.createChunk(currentChunk.id, combinedContent, currentChunk.title, currentChunk.type, currentChunk.level, currentChunk.id.split('_')[0]);
     ...
     * 관련 청크 찾기
     */
    findRelatedChunks(chunks, startIndex) {
        const relatedChunks = [chunks[startIndex]];
    ...
    adjustChunkSizes(chunks) {
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            if (chunk.wordCount < this.minChunkSize && i < chunks.length - 1) {
                ...
                const combinedChunk = this.createChunk(`${chunk.id}_combined`, combinedContent, chunk.title, chunk.type, chunk.level, chunk.id.split('_')[0]);
    ...
    splitLargeChunk(chunk) {
        ...
        for (const sentence of sentences) {
            ...
            if (currentWordCount + sentenceWordCount > this.maxChunkSize && currentContent.length > 0) {
                // 현재 청크 저장
                splitChunks.push(this.createChunk(`${chunk.id}_split_${splitIndex++}`, currentContent.join('. ') + '.', chunk.title, chunk.type, chunk.level, chunk.id.split('_')[0]));
     ...
     * 청크 간 관계 그래프 생성
     */
    buildChunkGraph(chunks, chunkGraph) {
        for (let i = 0; i < chunks.length; i++) {
            const chunk = chunks[i];
            const relatedChunks = [];
            // 이전 청크와의 관계
     ...
     * 의미적으로 유사한 청크 찾기
     */
    findSemanticallySimilarChunks(targetChunk, allChunks) {
        const similarChunks = [];
     ...
     * 청크 생성 헬퍼
     */
    createChunk(id, content, title, type, level, documentId) {
     ...
     * 의미적 점수 계산
     */
    calculateSemanticScore(content, keywords) {
        let score = 0;
        // 제목 키워드 가중치
