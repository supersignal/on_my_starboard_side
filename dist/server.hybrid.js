...
import { PaymentRequestSchema, CancelRequestSchema, PaymentStatusSchema, PaymentRequestZodSchema, CancelRequestZodSchema, PaymentStatusZodSchema, paymentTestService } from "./schemas/paymentService.js";
// ✅ Real Payment Service
import { RealPaymentRequestSchema, RealCancelRequestSchema, RealPaymentStatusSchema, RealPaymentRequestZodSchema, RealCancelRequestZodSchema, RealPaymentStatusZodSchema, createRealPaymentService } from "./schemas/realPaymentService.js";
// 환경변수 검증
const env = validateEnv();
console.log('환경변수 검증 완료:', env);
// 실제 결제 서비스 초기화 (설정이 있는 경우에만)
let realPaymentService = null;
try {
...
// MCP 툴 등록
mcpServer.tool("get_documents", `나이스페이먼츠 문서들을 버전 구분 없이 조회합니다.\n키워드 기반으로 v1, v2, general 문서를 모두 검색합니다.`, GetDocumentSchema, async ({ keywords }) => {
    return await getDocumentsByKeyword(keywords);
});
mcpServer.tool("document-details", `문서의 원본 ID 로 해당 문서의 전체 내용을 조회합니다.`, { id: z.string().describe("문서별 id 값") }, async ({ id }) => {
    try {
...
// 결제 테스트 MCP 도구들
mcpServer.tool("test_payment", `나이스페이먼츠 결제 API 테스트를 수행합니다.\n테스트 모드에서만 동작하며, 실제 결제는 발생하지 않습니다.`, PaymentRequestSchema, async (request) => {
    try {
...
mcpServer.tool("test_cancel", `나이스페이먼츠 결제 취소 API 테스트를 수행합니다.\n테스트 모드에서만 동작하며, 실제 취소는 발생하지 않습니다.`, CancelRequestSchema, async (request) => {
    try {
...
mcpServer.tool("test_payment_status", `나이스페이먼츠 결제 상태 조회 API 테스트를 수행합니다.\n테스트 모드에서만 동작합니다.`, PaymentStatusSchema, async (request) => {
    try {
...
mcpServer.tool("test_clear_data", `테스트 데이터를 초기화합니다.\n모든 테스트 결제 및 취소 내역이 삭제됩니다.`, {}, async () => {
    try {
...
mcpServer.tool("test_list_payments", `현재 저장된 모든 테스트 결제 내역을 조회합니다.`, {}, async () => {
    try {
...
// 실제 결제 MCP 도구들 (항상 등록, 설정에 따라 동작)
mcpServer.tool("real_payment", `나이스페이먼츠 실제 결제 API를 호출합니다.\n⚠️ 주의: 실제 결제가 발생합니다! testMode를 false로 설정해야 합니다.`, RealPaymentRequestSchema, async (request) => {
...
mcpServer.tool("real_cancel", `나이스페이먼츠 실제 결제 취소 API를 호출합니다.\n⚠️ 주의: 실제 취소가 발생합니다! testMode를 false로 설정해야 합니다.`, RealCancelRequestSchema, async (request) => {
    if (!realPaymentService) {
        return {
            content: [{
                    type: "text",
...
mcpServer.tool("real_payment_status", `나이스페이먼츠 실제 결제 상태 조회 API를 호출합니다.\n⚠️ 주의: 실제 API를 호출합니다! testMode를 false로 설정해야 합니다.`, RealPaymentStatusSchema, async (request) => {
    if (!realPaymentService) {
        return {
            content: [{
                    type: "text",
...
mcpServer.tool("real_payment_config", `나이스페이먼츠 실제 결제 서비스 설정을 조회합니다.`, {}, async () => {
    if (!realPaymentService) {
        return {
            content: [{
                    type: "text",
...
mcpServer.tool("real_payment_info", `실제 결제 서비스가 설정되지 않았습니다.\n환경변수 NICEPAY_MERCHANT_ID와 NICEPAY_MERCHANT_KEY를 설정해주세요.`, {}, async () => {
    return {
        content: [{
                type: "text",
                text: `실제 결제 서비스가 설정되지 않았습니다.\n\n설정 방법:\n1. 나이스페이먼츠와 계약 체결\n2. 상점 ID와 키 발급\n3. 환경변수 설정:\n   - NICEPAY_MERCHANT_ID=your_merchant_id\n   - NICEPAY_MERCHANT_KEY=your_merchant_key\n   - NICEPAY_TEST_MODE=false\n\n자세한 내용은 PAYMENT_TEST_GUIDE.md를 참고하세요.`
...
// 프롬프트 등록
mcpServer.prompt("java-spring-expert", "Java 21과 Spring Boot 전문가 프롬프트", {
...
// 결제 테스트 API 엔드포인트들
app.post('/mcp/test_payment', asyncHandler(async (req, res) => {
...
app.post('/mcp/test_cancel', asyncHandler(async (req, res) => {
...
app.post('/mcp/test_payment_status', asyncHandler(async (req, res) => {
...
app.post('/mcp/test_clear_data', asyncHandler(async (req, res) => {
    try {
        paymentTestService.clearTestData();
...
app.get('/mcp/test_list_payments', asyncHandler(async (req, res) => {
    try {
        const payments = paymentTestService.getAllTestPayments();
        const cancels = paymentTestService.getAllTestCancels();
        const result = {
...
// 실제 결제 API 엔드포인트들 (항상 등록, 설정에 따라 동작)
app.post('/mcp/real_payment', asyncHandler(async (req, res) => {
    if (!realPaymentService) {
        return res.status(400).json({
            isError: true,
            error: 'Real payment service not configured',
            content: [{
                    type: 'text',
                    text: `실제 결제 서비스가 설정되지 않았습니다.\n\n설정 방법:\n1. 나이스페이먼츠와 계약 체결\n2. 상점 ID와 키 발급\n3. 환경변수 설정:\n   - NICEPAY_MERCHANT_ID=your_merchant_id\n   - NICEPAY_MERCHANT_KEY=your_merchant_key\n   - NICEPAY_TEST_MODE=false\n\n자세한 내용은 PAYMENT_TEST_GUIDE.md를 참고하세요.`
                }]
...
app.post('/mcp/real_cancel', asyncHandler(async (req, res) => {
    if (!realPaymentService) {
        return res.status(400).json({
            isError: true,
            error: 'Real payment service not configured',
...
app.post('/mcp/real_payment_status', asyncHandler(async (req, res) => {
    if (!realPaymentService) {
        return res.status(400).json({
            isError: true,
            error: 'Real payment service not configured',
...
app.get('/mcp/real_payment_config', asyncHandler(async (req, res) => {
    if (!realPaymentService) {
        return res.status(400).json({
            isError: true,
            error: 'Real payment service not configured',
...
app.get('/mcp/real_payment_info', asyncHandler(async (req, res) => {
    res.json({
        content: [{
                type: 'text',
                text: `실제 결제 서비스가 설정되지 않았습니다.\n\n설정 방법:\n1. 나이스페이먼츠와 계약 체결\n2. 상점 ID와 키 발급\n3. 환경변수 설정:\n   - NICEPAY_MERCHANT_ID=your_merchant_id\n   - NICEPAY_MERCHANT_KEY=your_merchant_key\n   - NICEPAY_TEST_MODE=false\n\n자세한 내용은 PAYMENT_TEST_GUIDE.md를 참고하세요.`
