...
import { PaymentRequestSchema, CancelRequestSchema, PaymentStatusSchema, paymentTestService } from "./schemas/paymentService.js";
// ✅ Real Payment Service
import { RealPaymentRequestSchema, RealCancelRequestSchema, RealPaymentStatusSchema, createRealPaymentService } from "./schemas/realPaymentService.js";
//환경변수 로드 및 검증
...
server.tool("get_documents", `나이스페이먼츠 문서들을 버전 구분 없이 조회합니다.\n키워드 기반으로 v1, v2, general 문서를 모두 검색합니다.`, GetDocumentSchema, async ({ keywords }) => {
    // 키워드 기반 전체 문서 검색
    return await getDocumentsByKeyword(keywords);
});
server.tool("document-details", `문서의 원본 ID 로 해당 문서의 전체 내용을 조회합니다.`, { id: z.string().describe("문서별 id 값") }, async ({ id }) => {
    try {
...
server.tool("test_payment_status", `나이스페이먼츠 결제 상태 조회 API 테스트를 수행합니다.\n테스트 모드에서만 동작합니다.`, PaymentStatusSchema, async (request) => {
    try {
...
server.tool("test_clear_data", `테스트 데이터를 초기화합니다.\n모든 테스트 결제 및 취소 내역이 삭제됩니다.`, {}, async () => {
    try {
...
server.tool("test_list_payments", `현재 저장된 모든 테스트 결제 내역을 조회합니다.`, {}, async () => {
    try {
...
if (realPaymentService) {
    ...
    server.tool("real_payment_config", `나이스페이먼츠 실제 결제 서비스 설정을 조회합니다.`, {}, async () => {
        try {
